...
# In lightgbm, verbose is a model parameter but we can also set it in fit
...
model.fit(
    X[lib.TRAIN], Y[lib.TRAIN], 
    eval_set=eval_set, 
    # LightGBM handles early stopping inside fit via callbacks usually, 
    # but for simplicity we rely on the n_estimators in model_kwargs
)
